...
namespace ImpostorHqR.Core.Web.Http.Server.Response.Mime
{
    ...
    {
        private static readonly unsafe char* Lookup;

        static unsafe MimeProcessor()
        {
            var lower = new char[char.MaxValue - char.MinValue];
            for (var c = char.MinValue; c < char.MaxValue; c++)
            {
                lower[c] = char.IsLetter(c) ? char.ToLower(c) : c;
            }

            fixed (char* ptr = &lower[0]) Lookup = ptr;
        }
